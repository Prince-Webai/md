...
const Layout = ({ children }: { children: React.ReactNode }) => {
    ...
    const navSections = [
        {
            ...
            items: [
                ...
                ...(isAdmin ? [{ icon: Kanban, label: 'Pipeline', path: '/pipeline' }] : []),
    ...
    const mobileNavItems = [
        ...(isAdmin ? [{ icon: LayoutDashboard, label: 'Home', path: '/' }] : []),
    ...
    return (
        <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#F8FAFB] font-sans text-[#1a1a1a] relative">
            {/* Mobile Header */}
            <header className="md:hidden sticky top-0 z-[1000] bg-white border-b border-slate-200 h-[70px] flex items-center justify-between px-4 w-full">
        ...
        `} onClick={closeSidebar}>
            ...
            w-[280px] max-w-[85vw] transform transition-transform duration-300 flex flex-col overflow-y-auto
            ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
          `}>
                        {/* Mobile Sidebar Header */}
                        <div className="md:hidden flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
                            <span className="font-bold text-delaval-blue">Menu</span>
                            <button onClick={closeSidebar} className="p-2 -mr-2 text-slate-400 bg-slate-50 rounded-full hover:bg-slate-100">
                                <Menu size={20} />
                            </button>
                        </div>
