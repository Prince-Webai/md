...
import { Calendar, CheckCircle, Package, Settings, Users, ArrowUpRight, ArrowDownRight, Filter, Clock, Award } from 'lucide-react';
...
const Analytics = () => {
    ...
    const getFilterBounds = () => {
        ...
        if (timeRange === 'custom') {
            ...
            const startMs = now.getTime() - (timeRange as number) * 24 * 60 * 60 * 1000;
    ...
    const fetchMechanicStats = async () => {
        ...
        try {
            ...
            // Fetch labour_logs within range
            const { data: logs } = await supabase
                .from('labour_logs')
                .select('mechanic_id, duration_minutes, start_time')
                .gte('start_time', start)
                .lte('start_time', end);

            // Fetch completed jobs within range
            const { data: completedJobs } = await supabase
                .from('jobs')
                .select('engineer_name, actual_end_time')
                .eq('status', 'Completed')
                .gte('actual_end_time', start)
                .lte('actual_end_time', end);

            // Aggregate by mechanic
            const mechanicMap: Record<string, { name: string; totalMinutes: number; completedJobs: number }> = {};

            (logs || []).forEach((log: any) => {
                const name = log.mechanic_id || 'Unassigned';
                if (!mechanicMap[name]) mechanicMap[name] = { name, totalMinutes: 0, completedJobs: 0 };
                mechanicMap[name].totalMinutes += log.duration_minutes || 0;
            });

            (completedJobs || []).forEach((job: any) => {
                const name = job.engineer_name || 'Unassigned';
                if (!mechanicMap[name]) mechanicMap[name] = { name, totalMinutes: 0, completedJobs: 0 };
                mechanicMap[name].completedJobs += 1;
            });

            const result = Object.values(mechanicMap).map(m => ({
                ...m,
                totalHours: parseFloat((m.totalMinutes / 60).toFixed(1))
    ...
    const fetchAnalyticsData = async () => {
        try {
            ...
            // Using service_category for the "Top Service" metric and distribution
            const serviceCounts: any = {};
            jobs.forEach((j: any) => {
                const cat = j.service_category || 'General';
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto w-full pb-20 px-4 pt-4">
            ...
            <div className="section-card p-8">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-lg font-black text-slate-900">Mechanic Performance</h2>
                        <p className="text-sm font-medium text-slate-400">Hours logged and jobs completed per mechanic</p>
                    </div>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-delaval-blue uppercase tracking-widest bg-delaval-blue/10 px-3 py-1.5 rounded-full">
                        <Award size={12} /> Admin View
                    </div>
                </div>

                {mechanicLoading ? (
                    <div className="py-16 text-center text-slate-400 font-medium">Analyzing mechanic performance...</div>
                ) : mechanicStats.length === 0 ? (
                    <div className="py-16 text-center text-slate-400 font-medium">No mechanic data for this period.</div>
                ) : (
                    <div className="space-y-6">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="text-left">
                                        <th className="pb-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Mechanic</th>
                                        <th className="pb-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Jobs Completed</th>
                                        <th className="pb-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Hours Logged</th>
                                        <th className="pb-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Activity</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {mechanicStats.map((m: any, idx: number) => {
                                        const maxJobs = mechanicStats[0]?.completedJobs || 1;
                                        const pct = Math.round((m.completedJobs / maxJobs) * 100);
                                        return (
                                            <tr key={m.name} className="group">
                                                <td className="py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-9 h-9 rounded-xl bg-delaval-blue/10 text-delaval-blue flex items-center justify-center font-black text-xs">
                                                ...
                                                <td className="py-4 text-center">
                                                    <span className="px-3 py-1.5 rounded-lg bg-green-50 text-green-700 font-black text-xs">
                                                        {m.completedJobs} Jobs
                                                    </span>
                                                </td>
                                                <td className="py-4 text-center">
                                                    <span className="px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 font-black text-xs">
                                                        {m.totalHours} hrs
                                                    </span>
                                                </td>
                                                <td className="py-4">
                                                    <div className="flex items-center gap-2">
                                                        <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                                                            <div
                                                                className="h-full bg-[#0A8043] rounded-full transition-all duration-700"
                                                                style={{ width: `${pct}%` }}
                                                            />
                                                        </div>
                        ...
                        {mechanicStats.length > 0 && (
                            <div className="h-[200px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={mechanicStats} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                                        ...
                                        <RechartsTooltip
                                            content={({ active, payload }) => {
                                                if (active && payload && payload.length) {
                                                    return (
                                                        <div className="bg-white p-3 rounded-xl shadow-xl border border-slate-100 text-xs font-bold">
                                                            <p className="text-slate-500 mb-1">{payload[0].payload.name}</p>
                                                            <p className="text-[#0A8043]">{payload[0].value} hrs logged</p>
                                                        </div>
                                                    );
                                                }
                                                return null;
                                        ...
                                        />
                                        <Bar dataKey="totalHours" fill="#0A8043" radius={[6, 6, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        )}
