...
const JobDetails = () => {
    ...
    const formatDeadline = (date_completed: string) => {
        const end = new Date(date_completed).getTime();
    ...
    const formatLabour = (totalSeconds: number) => {
    ...
    const fetchJobDetails = async () => {
        ...
        if (error) console.error('Error fetching job:', error);
            ...
            setMechanicSignOff(data.mechanic_sign_off_name || data.engineer_name || '');
    ...
    const generateJobSheet = async () => {
        ...
        try {
            ...
            doc.setFontSize(22);
            doc.setFont('helvetica', 'bold');
            doc.text('INTERNAL JOB SHEET', 196, 22, { align: 'right' });
            ...
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(mutedText[0], mutedText[1], mutedText[2]);
            doc.text(`Generated: ${new Date().toLocaleDateString()}`, 196, 37, { align: 'right' });

            if (job.date_scheduled) {
                doc.setTextColor(220, 38, 38); // Red-ish for dates
                doc.setFont('helvetica', 'bold');
                doc.text(`Scheduled: ${new Date(job.date_scheduled).toLocaleDateString()}`, 196, 42, { align: 'right' });
            }
            ...
            doc.setFont('helvetica', 'normal');
            const custName = doc.splitTextToSize(job.customers?.name || 'N/A', 60);
            doc.text(custName, 45, 75);

            doc.setFont('helvetica', 'bold');
            doc.text('PHONE:', 20, 81 + ((custName.length - 1) * 4));
            doc.setFont('helvetica', 'normal');
            doc.text(job.customers?.phone || 'N/A', 45, 81 + ((custName.length - 1) * 4));

            // Column 2
            doc.setFont('helvetica', 'bold');
            doc.text('MACHINE:', 110, 75);
            doc.setFont('helvetica', 'normal');
            doc.text(`${job.machine_details || 'N/A'}${job.whole_good_number ? ` (WG: ${job.whole_good_number})` : ''}`, 140, 75);

            doc.setFont('helvetica', 'bold');
            doc.text('CATEGORY:', 110, 81);
            doc.setFont('helvetica', 'normal');
            doc.text(job.service_category || job.service_type || 'General Service', 140, 81);

            doc.setFont('helvetica', 'bold');
            doc.text('PRIORITY:', 110, 87);
            ...
            doc.text((job.priority || 'Normal').toUpperCase(), 140, 87);
            doc.setTextColor(textColor[0], textColor[1], textColor[2]); // Reset text color

            // --- Work Details ---
            drawSectionHeader('FAULT DESCRIPTION / INSTRUCTIONS', 105);

            // Description Box
            doc.setFillColor(secondaryBg[0], secondaryBg[1], secondaryBg[2]);
            doc.setDrawColor(borderColor[0], borderColor[1], borderColor[2]);
            doc.roundedRect(14, 113, 182, 35, 1, 1, 'FD');
            
            doc.setFont('helvetica', 'normal');
            const problemLines = doc.splitTextToSize(job.problem_description || job.notes || 'No description provided by customer.', 174);
            doc.text(problemLines, 18, 120);

            // --- Parts Table ---
            const tableY = 155;
            drawSectionHeader('RESERVED PARTS & MATERIALS', tableY);

            const reservedParts = items.filter(i => i.type === 'part');

            if (reservedParts.length > 0) {
                autoTable(doc, {
                    startY: tableY + 8,
                    head: [['DESCRIPTION', 'QTY', 'STATUS', 'CHECK']],
                    body: reservedParts.map(i => [
                        i.description,
                        i.quantity.toString(),
                        (i.status || 'Reserved').toUpperCase(),
                ...
                doc.setTextColor(mutedText[0], mutedText[1], mutedText[2]);
                doc.text('No parts are currently reserved for this job.', 14, tableY + 12);
            }

            // --- Quality Control / Mechanic Notes Section (For printing) ---
            const finalY = (doc as any).lastAutoTable ? (doc as any).lastAutoTable.finalY + 15 : tableY + 25;
            
            // If the table pushed us too far down, add a new page
            if (finalY > 230) {
                doc.addPage();
                doc.setFillColor(primaryColor[0], primaryColor[1], primaryColor[2]);
                doc.rect(0, 0, 210, 4, 'F');
                drawSectionHeader('MECHANIC NOTES & OUTCOME', 15);
                
                doc.setFillColor(255, 255, 255); // Force white
                doc.setDrawColor(borderColor[0], borderColor[1], borderColor[2]);
                doc.rect(14, 25, 182, 100);
            } else {
                drawSectionHeader('MECHANIC NOTES & OUTCOME', finalY);
                doc.setDrawColor(borderColor[0], borderColor[1], borderColor[2]);
                doc.rect(14, finalY + 10, 182, 60);
                
                // Signatures
                doc.setFontSize(9);
                doc.setFont('helvetica', 'normal');
                doc.setTextColor(mutedText[0], mutedText[1], mutedText[2]);
                doc.text('Mechanic Signature: _______________________', 14, finalY + 85);
                doc.text('Date: ___/___/20__', 150, finalY + 85);
            }
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.setFontSize(8);
                doc.setTextColor(mutedText[0], mutedText[1], mutedText[2]);
                doc.text('MD Burke Internal Workshop System', 14, pageHeight - 8);
                doc.text(`Ref: ${job.id.slice(0, 8)} | Page ${i} of ${pageCount}`, 196, pageHeight - 8, { align: 'right' });
            }
    ...
    const generateCompletionReport = async () => {
        ...
        try {
            ...
            doc.setFont('helvetica', 'normal');
            doc.text(job.service_category || 'General Service', 140, 87);
    ...
    // Labour billing: raw hours exist, round UP for billing
    const rawHours = job.total_hours_worked || 0;
    const billedHours = rawHours > 0 ? Math.ceil(rawHours) : 0;
    const LABOUR_RATE = 50; // €50/hr excl. VAT
    const VAT_RATE = 0.23;
    const labourExVAT = billedHours * LABOUR_RATE;
    const labourVAT = labourExVAT * VAT_RATE;
    const labourIncVAT = labourExVAT + labourVAT;
    const totalBillable = totalPartsCost + labourIncVAT;
                            ...
                            <div className="p-6">
                                {activeTab === 'items' && (
                                    <div className="space-y-6">
                                            ...
                                            <table className="w-full text-left">
                                                <thead className="bg-slate-50">
                                                    <tr className="text-left text-xs font-bold text-slate-500 uppercase tracking-widest">
                                                        ...
                                                        {isAdmin && <th className="px-4 py-3">Unit (€)</th>}
                                                ...
                                                <tbody>
                                                    {items.map(item => (
                                                        <tr key={item.id} className="border-t border-slate-100">
                                                            ...
                                                            {isAdmin && <td className="px-4 py-3 font-medium text-slate-600 text-sm">€{item.unit_price.toFixed(2)}</td>}
                                                            {isAdmin && <td className="px-4 py-3 text-right font-bold text-slate-900">€{(item.quantity * item.unit_price).toFixed(2)}</td>}
                                                            <td className="px-4 py-3">
                                        ...
                                        {job.status !== 'Completed' && (
                                            <div className="mt-8 border-2 border-slate-300 bg-slate-50 p-5 rounded-xl shadow-inner space-y-4 relative">
                                                <div className="absolute -top-3 left-4 bg-white px-3 py-0.5 text-[10px] font-black text-slate-500 uppercase tracking-widest border border-slate-300 rounded-md shadow-sm">
                                                    ➕ Add Part or Custom Item
                                                </div>
                                                <div className="flex gap-4">
                                                    <div className="flex-1">
                                                        <SearchableSelect
                                                            ...
                                                            className="px-4 py-2.5 rounded-xl border-2 border-black shadow-sm"
                                                        />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="relative pt-4">
                                                    ...
                                                    <div className="relative flex justify-center text-xs">
                                                        <span className="bg-slate-50 px-2 text-slate-500 italic font-medium">Or enter custom item details manually</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex gap-3 items-end">
                                                    ...
                                                    <div className="flex-1">
                                                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5 pl-1">Custom Description</label>
                                                        <input
                                                            className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:border-delaval-blue outline-none transition-all shadow-sm"
                                                            placeholder="E.g. One-off repair item..."
                                                            value={newItem.description}
                                                            onChange={e => setNewItem({ ...newItem, description: e.target.value })}
                                                        />
                                                    </div>
                                                    <div className="w-20">
                                                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5 pl-1">Qty</label>
                                                        <input
                                                            type="number"
                                                            className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm text-center focus:border-delaval-blue outline-none transition-all shadow-sm"
                                                    ...
                                                    {isAdmin && (
                                                        <div className="w-24">
                                                            <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5 pl-1">Unit (€)</label>
                                                            <input
                                                                type="number"
                                                                className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:border-delaval-blue outline-none transition-all shadow-sm"
                                                    ...
                                                    <button onClick={handleAddItem} className="bg-delaval-blue text-white w-12 h-[42px] rounded-lg flex items-center justify-center hover:bg-delaval-dark-blue shadow-md transition-all active:scale-95 mb-[1px]">
                                                        <Plus size={20} />
                                                    </button>
                                                </div>
                                            </div>
                                                ...
                                                {history.map(h => (
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-start mb-2">
                                                            ...
                                                            <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${h.status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
                                                                }`}>
                                                                {h.status}
                                                            </span>
                                                        </div>
                                                    ...
                                                    <div className="text-xl font-black font-mono flex items-center gap-2">
                                                        {job.status === 'Completed' ? (
                                                            ...
                                                            <>
                                                                ...
                                                                {job.date_completed ? formatDeadline(job.date_completed) : formatLabour(timerStatus === 'running' ? elapsedSeconds : 0)}
                                                            </>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                <div className="space-y-4">
                                        ...
                                        <div className="bg-white p-5 rounded-2xl border border-slate-100 space-y-5 shadow-sm">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 flex justify-between items-center">
                                                    Live Labour Logs
                                                    <span className="font-mono text-delaval-blue bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
                                                        {formatLabour(timerStatus === 'running' ? elapsedSeconds : 0)}
                                                    </span>
                                                </label>
                                                <div className="flex gap-2">
                                                    {timerStatus === 'running' ? (
                                                        <button
                                                            ...
                                                            className="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 text-xs uppercase tracking-widest border-b-4 border-amber-700 active:border-b-0"
                                                        >
                                                            <Pause size={18} /> Pause
                                                        </button>
                                                    ) : (
                                                        <button
                                                            ...
                                                            className="flex-1 bg-delaval-blue hover:bg-[#065F30] text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 text-xs uppercase tracking-widest border-b-4 border-[#044020] active:border-b-0"
                                                        >
                                                            <Play size={18} /> Start
                                                        </button>
                                                    )}
                                                    <button
                                                        ...
                                                        className="flex-1 bg-[#0A8043] hover:bg-[#065F30] text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 text-xs uppercase tracking-widest border-b-4 border-[#065F30] active:border-b-0"
                                                        title="Complete Job & Stop Timer"
                                                    >
                                    ...
                                    {isAdmin && (
                                        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                                            <div className="px-5 py-4 border-b border-slate-100">
                                                <h3 className="font-bold text-slate-900 flex items-center gap-2"><Receipt size={16} className="text-[#0A8043]" /> Job Profitability</h3>
                                            </div>
                                            <div className="px-5 py-4 space-y-3 text-sm">
                                                <div className="flex justify-between">
                                                    <span className="text-slate-500 font-medium">Parts Cost</span>
                                                    <span className="font-bold text-slate-900">€{totalPartsCost.toFixed(2)}</span>
                                                </div>
                                                {rawHours > 0 && (
                                                    <>
                                                        <div className="flex justify-between">
                                                            <span className="text-slate-500 font-medium">Labour ({rawHours.toFixed(2)}h → {billedHours}h billed)</span>
                                                            <span className="font-bold text-slate-900">€{labourExVAT.toFixed(2)}</span>
                                                        </div>
                                                        <div className="flex justify-between text-xs">
                                                            <span className="text-slate-400">VAT @ 23%</span>
                                                            <span className="text-slate-500">€{labourVAT.toFixed(2)}</span>
                                                        </div>
                                                        <div className="flex justify-between">
                                                            <span className="text-slate-500 font-medium">Labour (inc. VAT)</span>
                                                            <span className="font-bold text-slate-900">€{labourIncVAT.toFixed(2)}</span>
                                                        </div>
                                                    </>
                                                )}
                                                <div className="flex justify-between pt-3 border-t border-slate-100">
                                                    <span className="font-black text-slate-900">Estimated Total</span>
                                                    <span className="font-black text-[#0A8043] text-base">€{totalBillable.toFixed(2)}</span>
                                                </div>
                                                <p className="text-[10px] text-slate-400 font-medium">Labour: €{LABOUR_RATE}/hr excl. VAT · Partial hours rounded UP</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {job.status === 'Completed' && (
                                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                                            ...
                                            <div>
                                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 pl-1">1. Select Job Category (Standardised Description)</label>
                                                <div className="relative mb-4">
                                                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
                                                        <Wrench size={16} />
                                                    </div>
                                                    <select
                                                        className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-delaval-blue transition-all appearance-none cursor-pointer shadow-sm"
                                                        value={serviceCategory}
                                                        onChange={e => {
                                                            const val = e.target.value;
                                                            setServiceCategory(val);
                                                            if (!repairSummary) {
                                                                if (val === 'Full Service') setRepairSummary('Completed a comprehensive full service as per manufacturer guidelines. All fluid levels checked and replaced, filters renewed. Machine tested and operating normally.');
                                                                if (val === 'Minor Service') setRepairSummary('Completed a minor routine service. Essential checks performed and lubricated. Functional tests passed.');
                                                                if (val === 'Repair') setRepairSummary('Investigated and repaired reported fault. Replaced defective components. System tested and returned to normal operation.');
                                                            }
                                                        }}
                                                    >
                                                ...
                                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 pl-1">2. Detailed Repair Summary (Editable)</label>
                                                <textarea
                                                    className="w-full p-4 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-delaval-blue transition-all min-h-[100px] shadow-sm mb-4"
                                                    placeholder="Enter work performed. Auto-fills based on category above..."
                                                    value={repairSummary}
                                            ...
                                            <Fragment>
                                                ...
                                                <div className="space-y-4 pt-4 border-t border-slate-100 mt-4">
                                                    ...
                                                    <div className="col-span-1 md:col-span-2 mt-2">
                                                        <label className="block text-sm font-medium text-slate-500 mb-1">Fault Description / Instructions</label>
                                                        <textarea
                                                            className="w-full text-slate-900 bg-white border border-slate-200 p-3 rounded-lg text-sm shadow-inner transition-colors focus:border-delaval-blue outline-none focus:ring-2 focus:ring-delaval-blue/20"
                                                            rows={4}
                                                            placeholder="State the fault reported or service required..."
                                                            value={job.notes || ''}
                                                            onChange={(e) => setJob({ ...job, notes: e.target.value })}
                                                            onBlur={async () => {
                                                                await supabase.from('jobs').update({ notes: job.notes }).eq('id', job.id);
                                                                showToast('Updated', 'Fault Description saved successfully.', 'success');
                                                            }}
                                                        />
                                                    </div>
                                                </div>
                    ...
                    <div className="pt-2">
                                ...
                                {job.status === 'Completed' && (
                                    <div className="mt-6 space-y-3">
                                        ...
                                        <div className="bg-white p-4 rounded-xl border border-slate-100 space-y-4">
                                            ...
                                            />
                                            <select
                                                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-delaval-blue transition-all appearance-none cursor-pointer"
                                                value={serviceCategory}
                                            ...
                                            >
                                                ...
                                                <option value="Repair – Mechanical Fault">Repair – Mechanical Fault</option>
                                                <option value="Repair – Electrical Fault">Repair – Electrical Fault</option>
                                                <option value="Emergency Breakdown">Emergency Breakdown</option>
                                                <option value="Warranty Claim">Warranty Claim</option>
                                                <option value="Installation">Installation</option>
                                                <option value="General Service">General Service</option>
                        ...
                        {mobileTab === 'parts' && (
                            <div className="space-y-4">
                                ...
                                {job.status !== 'Completed' && (
                                    <div className="bg-slate-50 p-5 rounded-xl shadow-inner border-2 border-slate-300 mb-6 space-y-4 relative mt-4 z-50 overflow-visible">
                                        <div className="absolute -top-3 left-4 bg-white px-3 py-0.5 text-[10px] font-black text-slate-500 uppercase tracking-widest border border-slate-300 rounded-md shadow-sm">
                                            ➕ Add Part / Custom Item
                                        </div>
                                        <div className="mt-2 relative z-50 overflow-visible">
                                            <SearchableSelect
                                                label="Search Catalog"
                                                options={inventory.map(inv => ({ value: inv.id, label: `${inv.name} (€${inv.sell_price})` }))}
                                                value=""
                                        ...
                                        <div className="relative py-2">
                                            ...
                                            <div className="relative flex justify-center text-xs">
                                                <span className="bg-slate-50 px-2 text-slate-500 italic font-medium">Or enter manual details</span>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5 pl-1">Custom Description</label>
                                            <input
                                                className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm focus:border-delaval-blue outline-none"
                                        ...
                                        <div className="flex gap-3">
                                            <div className="w-1/3">
                                                ...
                                                <input
                                                    type="number"
                                                    className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm text-center shadow-sm focus:border-delaval-blue outline-none"
                                                    value={newItem.quantity}
                                                    onChange={e => setNewItem({ ...newItem, quantity: Number(e.target.value) })}
                                                />
                                            </div>
                                            <div className="w-1/3">
                                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5 pl-1">Price (€)</label>
                                                <input
                                                    type="number"
                                                    className="w-full p-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm focus:border-delaval-blue outline-none"
                                                    value={newItem.unit_price}
                                            ...
                                            <div className="w-1/3 flex items-end">
                                                <button
                                                    ...
                                                    className="w-full bg-delaval-blue text-white p-2.5 rounded-lg flex items-center justify-center hover:bg-delaval-dark-blue h-[42px] shadow-md"
                                                >
                                                    <Plus size={20} />
                                                </button>
                                            </div>
